/**
 * api/db-setup.ts — ONE-TIME endpoint to initialise / patch the Neon PostgreSQL tables.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const STATEMENTS = [
    ...
    `DO $$ BEGIN CREATE TYPE notification_type AS ENUM ('SYSTEM','TRANSACTION','BROADCAST','SUPPORT','PROMO'); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    ...
    `CREATE TABLE IF NOT EXISTS linked_wallets (
      ...
      updated_at TIMESTAMP NOT NULL DEFAULT NOW(), UNIQUE (user_id, address, chain_id)
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS account_audit_logs (
      id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      action VARCHAR(128) NOT NULL, details JSONB, metadata JSONB, ip_address VARCHAR(45),
      timestamp TIMESTAMP NOT NULL DEFAULT NOW(), created_at TIMESTAMP NOT NULL DEFAULT NOW()
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS portfolio_snapshots (
      ...
      total_value_usd NUMERIC(20,2) NOT NULL DEFAULT 0, chain_count INTEGER NOT NULL DEFAULT 0,
      asset_count INTEGER NOT NULL DEFAULT 0, snapshot JSONB NOT NULL,
    ...
    // NOTIFICATIONS (full schema with all columns)
    `CREATE TABLE IF NOT EXISTS notifications (
      ...
      sent_by_admin INTEGER REFERENCES users(id),
      data JSONB,
      created_at TIMESTAMP NOT NULL DEFAULT NOW()
    )`,
    `CREATE INDEX IF NOT EXISTS idx_notif_user ON notifications(user_id)`,

    // Add missing cols to notifications if table already existed without them
    `ALTER TABLE notifications ADD COLUMN IF NOT EXISTS action_url VARCHAR(512)`,
    `ALTER TABLE notifications ADD COLUMN IF NOT EXISTS sent_by_admin INTEGER`,
    `ALTER TABLE notifications ADD COLUMN IF NOT EXISTS data JSONB`,

    // Ensure audit_logs has details + metadata
    `ALTER TABLE account_audit_logs ADD COLUMN IF NOT EXISTS details JSONB`,
    ...
    for (const stmt of STATEMENTS) {
      try { await pool.query(stmt); ok++; }
        ...
        if (!e.message?.includes("already exists") && !e.message?.includes("duplicate")) {
          errs.push(e.message?.slice(0, 150));
        }
      }
    }
    await pool.end();
    return res.json({ ok: true, statements: ok, warnings: errs.length, errors: errs.length > 0 ? errs : undefined });
  } catch (e: any) {
