 ...
 * Shows personal + broadcast messages, marks them read on open
 */
    ...
    <div className="max-w-xl mx-auto px-4 py-6 space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-card border border-border rounded-2xl p-10 text-center space-y-3">
          ...
          <p className="text-sm font-medium text-aegis-primary-dark dark:text-white">No notifications yet</p>
          <p className="text-xs text-aegis-tertiary-dark">System updates and admin messages will appear here</p>
        </div>
      ) : (
            ...
            return (
              <div key={n.id}
                ...
                className={`bg-card border rounded-xl p-4 flex gap-3 ${
                  n.actionUrl ? "cursor-pointer hover:border-[#5B3CF5]/50 transition-colors" : ""
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-semibold text-aegis-primary-dark dark:text-white">{n.title}</p>
                    {!n.isRead && <span className="w-2 h-2 bg-[#5B3CF5] rounded-full flex-shrink-0 mt-1.5" />}
                  </div>
